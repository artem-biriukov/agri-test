...
def load_pdfs(pdf_dir: str, collection_name: str, chunk_size: int = 1000, chunk_overlap: int = 100):
    ...
    try:
        embed_model = VertexTextEmbedding(
            model_name=EMBEDDING_MODEL, project=GCP_PROJECT, location=GCP_LOCATION, embed_batch_size=1
        )
    except Exception as e:
    ...
    try:
        documents = SimpleDirectoryReader(
            input_dir=str(pdf_path), file_extractor={".pdf": None}, recursive=True  # Use default PDF extractor
        ).load_data()
