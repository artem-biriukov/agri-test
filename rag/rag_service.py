...
async def chat(message: ChatMessage):
    ...
    stress_info = (
        f"CSI={agri_data.get('csi_overall')}, Water stress={agri_data.get('water_stress')}%, Heat stress={agri_data.get('heat_stress')}°C\n"
        if agri_data
        else ""
    )
...
Answer:"""
    ...
    response = model.generate_content(
        prompt, generation_config=genai.types.GenerationConfig(max_output_tokens=1024, temperature=0.3)
    )

    return ChatResponse(
        response=response.text, retrieved_contexts=[], model=GEMINI_MODEL, timestamp=datetime.now().isoformat()
    )
