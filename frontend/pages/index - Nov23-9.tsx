...
export default function Home() {
  ...
  const fetchData = useCallback(async (countyCode) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8000/mcsi/county/${countyCode}/timeseries`
      );
  ...
  const chartData = timeseries.map(item => ({
    ...
    date: item.week_start?.substring(5, 10),
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-50">
      ...
      <div className="max-w-7xl mx-auto px-8 py-12">
              ...
              >
                {uniqueWeeks.map(w => (
                  <option key={w} value={w}>
                    Week {w} - {allTimeseries.find(item => item.week_of_season === w)?.week_start?.substring(0, 10)} {WEEK_HINTS[w] ? `(${WEEK_HINTS[w]})` : ''}
                  </option>
                ))}
              </select>
            </div>
        ...
        {loading ? (
          ...
          <>
            <div className="bg-white p-8 rounded-lg shadow mb-8">
              <h2 className="text-2xl font-bold text-gray-900 mb-6">Corn Stress Index (MCSI)</h2>
