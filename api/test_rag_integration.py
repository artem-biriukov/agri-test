...
def rag_request(sample_mcsi_data, sample_yield_data):
    ...
    return RAGRequest(mcsi_data=sample_mcsi_data, yield_data=sample_yield_data, question=None)
...
def test_yield_data_validation(sample_yield_data):
    ...
    assert (
        sample_yield_data.confidence_interval_lower
...
def test_interpret_endpoint(rag_request):
    ...
    response = client.post("/interpret", json=rag_request.dict())
...
def test_batch_interpret(sample_mcsi_data, sample_yield_data):
    ...
    requests_data = [
        {"mcsi_data": sample_mcsi_data.dict(), "yield_data": sample_yield_data.dict(), "question": None},
        {
...
def test_response_time(sample_mcsi_data, sample_yield_data):
    ...
    request_data = {"mcsi_data": sample_mcsi_data.dict(), "yield_data": sample_yield_data.dict(), "question": None}
